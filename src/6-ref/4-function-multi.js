...
export default function FunctionMulti() {
  ...
  function onClick() {
    ...
    for (const box of BOX_LIST) {
      ...
      if (ref) {
        const rect = ref.getBoundingClientRect(); // 사각형의 정보
        if (maxRight < rect.right) { // rect.right 가장 끝의 정보 
          maxRight = rect.right; // 가장 큰 값으로 처리
          maxId = box.id; // 해당 box의 id값
        }
      }
    }
